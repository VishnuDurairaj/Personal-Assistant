        ...
        if tool_details['tool_name'] in self.tool_names :
            ...
            if tool_details['tool_name'] == 'AssignTask':
                ...
                try:
                    ...
                    elements.append(cl.Text(name=f"{self.agent_name} : Calling Tool {tool_details['tool_name']}", content=str(tool_details['tool_args']), display="inline"))
                    ...
                    if tool_details['tool_name'] == 'ImageGenerator' and len(tool_output):

                        tool_content=f"The illustration has been generated and displayed to the students. You can explain using the illustration"

                        elements.append(cl.Image(name=f"Illustration", path=tool_output.strip(), display="inline"))

                        asyncio.run(cl.Message("Illustration",elements=elements).send())
...
from core.vectorestores import ChromaDB
...
async def start_message():
    ...
    class AddInformation(BaseModel):

        "This tool is used to add information to the database. It stores the data in such a way that it is easy to retrieve the full context later."

        data : str = Field(description="The information you want to store. You must format the data so that it maintains its context for retrieval in the future. Include any relevant details like source, date, time, and description to enhance the stored information's usefulness.")
        
        def run(self):

            response = vector_db.add_documents([self.data])

            return response

    class GetInformation(BaseModel):

        "This tool is used to retrieve the top 5 relevant pieces of data from the database based on a user-provided query."

        question : str = Field(description="The question or context you're looking to retrieve information for. It could be a partial detail or a general question about what was previously stored.")

        def run(self):

            response = vector_db.retrieve_top_n(self.question)

            return response          

    class DeleteInformation(BaseModel):

        "This tool is used to delete specific information from the database based on the provided ID."

        id : str = Field(description="The unique ID of the data you want to delete from the database. You must retrieve this ID using the GetInformation tool before using this tool.")

        def run(self):

            response = vector_db.delete_document(self.id)

            return response

    class UpdateInformation(BaseModel):

        "This tool is used to update the existing information stored in the database with new data."

        id : str = Field(description="The unique ID of the existing data you wish to update. Retrieve this ID using the GetInformation tool first.")

        new_data : str = Field(description="The new information you want to update the existing data with. Ensure that the new data is comprehensive and includes all necessary updates.")

        def run(self):

            response = vector_db.update_document(self.id,self.new_data)

            return response           

    class JupyterNotebookTool(BaseModel):

        """This tool allows the execution of Python code in a stateful Jupyter notebook environment. It helps users run Python scripts to accomplish tasks that can be down through coding."""
    ...
    tools = [AddInformation,GetInformation,DeleteInformation,UpdateInformation,JupyterNotebookTool]

    model = OpenaiChatModel(model_name="gpt-4o-mini",temperature=0.5,verbose=True)

    # model_name= 'claude-3-5-sonnet-20240620'

    # model = AnthropicModel(api_key=api_key)

    agent = ChainlitStructuredAgent(model,"AI Assistant",description,instruction,tools,max_allowed_attempts=40)
...
if __name__ == "__main__":
    ...
    subprocess.run(["chainlit", "run", "assistant.py", "--port", "8088"])